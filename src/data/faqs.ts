export const faqsData = {
    es: [
        {
            question: "¿En qué consiste el trabajo de consultoría de Surcos?",
            answer: "Acompañamos a instituciones en el diseño, la planificación estratégica y evaluación de proyectos, aplicando nuestra amplia experiencia en consultoría educativa en Argentina. Brindamos asesoramiento externo para lograr un impacto a largo plazo y optimizar la toma de decisiones directivas.",
        },
        {
            question: "¿Para quiénes están pensados los servicios formativos?",
            answer: "Nuestra vasta experiencia en formación para organizaciones sociales está dirigida a educadores, referentes territoriales y comunitarios, incluyendo la capacitación de equipos pastorales y directivos. Están diseñados para liderazgos que buscan actualizar estrategias pedagógicas y mejorar su capacidad de respuesta a los desafíos del siglo XXI.",
        },
        {
            question: "¿Cómo es el proceso de creación de contenidos educativos?",
            answer: "Ideamos desde cero o potenciamos tus materiales didácticos, guías de estudio y recursos. Como editorial educativa en Argentina, nuestro equipo gestiona la curación académica, el diseño y la adaptación didáctica, distribuyendo recursos de excelencia en formatos impresos o en los ecosistemas digitales actuales.",
        },
        {
            question: "¿Brindan servicios para instituciones de otros países?",
            answer: "Brindamos nuestros servicios de planificación estratégica para organizaciones sociales y asesoramiento pastoral institucional en red con fundaciones y escuelas en toda América Latina. Hoy, las barreras geográficas no representan ninguna limitación para el enorme alcance, eficacia y acompañamiento cotidiano de nuestro experimentado equipo de consultores.",
        },
        {
            question: "¿Qué alcance tiene la 'Gestión editorial completa'?",
            answer: "Nos hacemos cargo de todo el flujo de pre-impresión institucional. El cuidado abarca la revisión académica, corrección de estilo y el diseño de materiales pedagógicos, hasta la gestión legal del ISBN y su preparación final para imprentas o formatos digitales.",
        },
    ],
    en: [
        {
            question: "What does Surcos' consulting work involve?",
            answer: "We accompany institutions in the precise design, strategic planning, and evaluation of projects, bringing our extensive expertise in educational consulting in Latin America. We provide expert external advisory to significantly improve decision-making and achieve positive long-term community impact.",
        },
        {
            question: "Who are the training services designed for?",
            answer: "Leveraging our experience in training for social organizations, our courses empower educators, grassroots leaders, and the comprehensive training of pastoral teams. They are specially designed for leadership roles seeking to update pedagogical strategies and improve their critical response capacity toward twenty-first-century challenges.",
        },
        {
            question: "What is the process of creating educational materials like?",
            answer: "We conceive didactic materials and study guides from scratch or enhance existing content. As a top educational publisher in Latin America, our team handles academic curation, elegant design, and didactic adaptation, delivering marvelous resources in physical print or modern digital environments.",
        },
        {
            question: "Do you provide services for institutions outside Argentina?",
            answer: "We proudly provide our services in strategic planning for social organizations and profound institutional pastoral advisory remotely alongside foundations across Latin America. Today, traditional geographical borders do not represent any limitation to the massive reach, proven effectiveness, and warm accompaniment of our experienced team.",
        },
        {
            question: "What does 'Full editorial management' cover?",
            answer: "We direct the complete pre-press literary workflow for your institution. This rigorous process ranges from academic review and copyediting to the visual design of pedagogical materials, plus ISBN registration and technical preparation for physical printing or digital dissemination.",
        },
    ],
} as const;
